...
public class BoxDiagram extends GraphicsProgram {
	...
	private void createController() {
		nameFeild = new JTextField (MAX_NAME);
	...
	private void addBox (String name) {
		GCompund box = new GCompound();
		GRect outline = new GRect (BOX_WODTH, BOX_HEIGHT);
		GLabel label = new GLabel (name);
		box.add (outline, -BOX_WIDTH / 2, label.getAscent()) / 2);
	...
	public void actionPerformed (ActionEvent e) {
		...
		if (source== addButtton || source == nameField) {
			...
			/* Called on mouse press to record the coordinates of the click */ public void mousePressed(MouseEvent e) {
			      last = new GPoint(e.getPoint());
			      currentObject = getElementAt(last);
			   }
			/* Called on mouse drag to reposition the object */ public void mouseDragged(MouseEvent e) {
			if (currentObject != null) { currentObject.move(e.getX() - last.getX(),
			                            e.getY() - last.getY());
			         last = new GPoint(e.getPoint());
			} }
			/* Called on mouse click to move this object to the front */ public void mouseClicked(MouseEvent e) {
			if (currentObject != null) currentObject.sendToFront(); }
			/* Private constants */
			private static final int MAX_NAME = 25; private static final double BOX_WIDTH = 120; private static final double BOX_HEIGHT = 50;
